...
def is_convex(poly):
    ...
    for i in range(0,len(poly)):
        dx1 = poly[j]['x'] - poly[k]['x']
        dy1 = poly[j]['y'] - poly[k]['y']
        dx2 = poly[i]['x'] - poly[j]['x']
        dy2 = poly[i]['y'] - poly[j]['y']
        zcrossproduct.append(dx1*dy2 - dy1*dx2)
        
        j,k = i,j
    
    return all([z >= 0 for z in zcrossproduct]) or all([z <= 0 for z in zcrossproduct])
...
def D_function(a,b,p):
            return (a['x']-b['x'])*(a['y']-p['y']) - (a['y']-b['y'])*(a['x']-p['x'])        
        
def edge_point_distance(edge,point):
    if edge[0]['x'] == edge[1]['x']:
        return abs(point['x'] - edge[0]['x'])
    
    else:
        slope = (edge[1]['y']-edge[0]['y'])/(edge[1]['x']-edge[0]['x'])
        
        return abs( (point['y'] + slope*(edge[0]['x']-point['x']) - edge[0]['y'])/math.sqrt( 1 + slope**2 )  )
